...
import { testAPIConnection, type TranslationProvider } from "../api/custom-translator";
...
function buildPrefsPane() {
  ...
  ztoolkit.UI.replaceElement(
    {
      tag: "menulist",
      id: `${config.addonRef}-custom-provider`,
      attributes: {
        value: getPref("customProvider") as string || "openai",
        native: "true",
      },
      styles: {
        maxWidth: "250px",
      ...
      children: [
        {
          tag: "menupopup",
          children: customProviders.map((item) => {
            return {
              tag: "menuitem",
              attributes: {
                label: item.label,
      ...
      listeners: [
        {
          ...
          listener: (e: Event) => {
            ztoolkit.log(e);
            setPref("customProvider", (e.target as XUL.MenuList).value);
          },
        },
      ],
    },
    doc.querySelector(`#${config.addonRef}-custom-provider-placeholder`)!,
  );
}
...
function bindPrefEvents() {
  ...
  addon.data
    .prefs!.window.document?.querySelector(
      `#zotero-prefpane-${config.addonRef}-custom-apikey`,
    )
    ?.addEventListener("change", (e: Event) => {
      ztoolkit.log(e);
  ...
  addon.data
    ...
    ?.addEventListener("change", (e: Event) => {
      ztoolkit.log(e);
      setPref("customModel", (e.target as HTMLInputElement).value);
    });

  // Bind custom API test button
  addon.data
    .prefs!.window.document?.querySelector(
      `#zotero-prefpane-${config.addonRef}-test-custom-api-button`,
    )
    ?.addEventListener("command", async () => {
      try {
        ...
        if (!provider || !apiKey) {
          showDialog({
            title: getString("pref-test-failed"),
            message: "Please configure provider and API key first",
          });
          return;
        }
